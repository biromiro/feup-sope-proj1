...
int get_new_perms(const char *pathname, perm_operation_t *permissions, mode_t *new_perm) {
    ...
    for (size_t offset = 0; offset < 3; offset++) {
        ...
        int current_r = (current_perms / READ_VAL) * READ_VAL, rest_r = current_perms % READ_VAL;
        ...
        int new_r = (permissions->permission_octal / READ_VAL) * READ_VAL;
        rest_r = permissions->permission_octal % READ_VAL;
